...
% Explicacion: Aqui recibe un conjunto (id de los estudiantes),
% Objetivos(una list de estudiantes y si el curso ingresado forma parte
% de los suyos(true/false)) y la lista que recibira los estudiantes que
% llevan el ccurso...NB (clausulas): el primer assera fija encontrar
% como una lista vacia, call llama o establece las metas u objetivos a
% cumplirse es decir: agrega a la lista "miembros" los estudiantes
% quienes llevan el curso ingresado por el usuario, fail indica que todo
% esto puede fallar, por lo que no es necesario ponerlo, cabe destacar
% que ese conjunto encontrar ya queda en la base para ser llamado por
% otros predicados. se llama a encontrado enviando una lista vacia y la
% lista receptora.
busqueda(Grupo, Objetivos, Lista_estudiantes):- asserta( miembros( [] ) ),
        call(Objetivos), asserta( miembros( {Grupo} ) ), fail;
        alumnos_encontrados([], Lista_estudiantes).
% en la primera llamada es una lista vacia y Lista la cual recibira los
% ID estudiantes...
% --------------------------------------------------------------------
% Explicacion: Aqui recibe dos listas vacias por asi decirlo, retract
% extrae un elemnto de el conjunto encontrar previamente establecido, lo
% extra ya que en la siguiente iteracion no deseamos usarlo de nuevo, el
% signo ! puta idea que es , se vuelve a llamar "alumnos_encontrados
% esta vez con elemento extraido como parametro, lista vacia y la
% lista receptora"
alumnos_encontrados(Lista_vacia, Lista_estudiantes):- retract( miembros( Miembro ) ), !, alumnos_encontrados( Miembro, Lista_vacia, Lista_estudiantes).
% Recuerdan asserta(miembros([]))? pues es una lista vavcia agregada
% junto con el conjunto, cuando se llega a esta lista vacia se devuelve
% la lista de id acumulada.
alumnos_encontrados([], Lista, Lista).
% Explicacion: Aqui recibe el conjunto estudiante (id,zur/der, lista de
% cursos). Cuando se vuelve a llamar, segun yo
% "[Miembro|Lista_vacia]=[id estudiante| resto de informacion que no nos
% interesa], recuerden que Lista_vacia=[] lista vacia. Entonces eso se
% val almcaenando recursivamente."
alumnos_encontrados({Miembro}, Lista_vacia, Lista):- alumnos_encontrados([Miembro|Lista_vacia], Lista).

uniendo([],[]).
uniendo([ID | IDS], F) :- fechaexam(ID, F1), uniendo(IDS, F2), append(F1, F2, F).

tamano([],0).
tamano([_|Y], R) :- tamano(Y, R1), R is R1 + 1.

lista_estudiantes_zurdos(CURSO, LISTA_ESTUDIANTES):- busqueda(ESTUDIANTES, (estudiante(ESTUDIANTES, 0, CURSOS), member(CURSO, CURSOS)), LISTA_ESTUDIANTES).

%% --------------------PREDICADOS A CONSULTAR-------------------

%% PREDICADO #1
%% Predicado lista_estudiantes_por_curso(id_curso, lista_e). Toma el id
%% del curso y agrega a lista_e los estudiantes que toman el curso.
%% ------------------------------------------------------------------------
%% EXPLICACION: La llamada a busqueda incluye los siguientes parametros:
%% 1. ESTUDIANTE: que es ek cojunto o coleccion de IDs de los
%% estudiantes. 2-Metas [(estudiante(ESTUDIANTE, _, CURSOS),
%% member(CURSO, CURSOS))]: es el conjunto de estudiantes con todos los
%% datos, y el boolean si el curso ingresado por el usuario pertenece a
%% los cursos de cada estudiante, esto se ocupara mas adelante. 3-
%% ESTUDIANTES:Lista receptora donde se guardara los resultados.
lista_estudiantes_por_curso(CURSO, ESTUDIANTES):- busqueda(ESTUDIANTE, (estudiante(ESTUDIANTE, _, CURSOS), member(CURSO, CURSOS)), ESTUDIANTES).

%% PREDICADO #2
%% Predicado lista_fechas_de_examenes(id_estudiante, lista_fechas)
%% Toma el id del estudiante y agrega a lista de fechas de las fechas de
%% los examenes de los cursos que le corresponde.
lista_fechas_de_examenes(Id_estudiante, Fechas_examenes):- estudiante(Id_estudiante,_,Cursos), uniendo(Cursos, Fechas_examenes).

%% PREDICADO #3
%% Predicado aulas_adecuadas_para_zurdos(id_curso, lista_aulas)
...
aulas_adecuadas_para_zurdos(Id_curso, Lista_de_aulas):- busqueda(AULA, (aula(AULA, _, Cantidad_asientos_de_zurdos), lista_estudiantes_zurdos(Id_curso, Zurdos_en_el_curso), tamano(Zurdos_en_el_curso, Cantidad_total_zurdos_del_curso), Cantidad_asientos_de_zurdos>= Cantidad_total_zurdos_del_curso), Lista_de_aulas).
